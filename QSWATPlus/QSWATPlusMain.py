...
#from .main16218_v02_v02 import SWATApp
...
class QSWATPlus(QObject):
    ...
    def initButtons(self):
        ...
        self._odlg.migrateclimateButton.clicked.connect(self.abrirModuloClima)
        #self._odlg.donwloadclimateButton.clicked.connect(self.abrirDescargaClimatica)
        self._odlg.donwloadclimateButton.clicked.connect(self.abrirDescargaClimatica)

    def abrirModuloClima(self):
        from script12_main import VentanaPrincipal

        # Crea instancia de tu ventana
        self.ventanaClima = VentanaPrincipal()
        self.ventanaClima.show()

    """def abrirDescargaClimatica(self):
        self.ventanaClima = SWATApp()
        self.ventanaClima.show()"""



    def abrirDescargaClimatica(self):
        ruta_exe = r"C:\Users\jhonv\Desktop\S07\CodeGUIsolver\dist\main16218_v02_v03.exe"

        try:
            print(f"Lanzando: {ruta_exe}")
            subprocess.Popen([ruta_exe], shell=True)
        except Exception as e:
            from PyQt5.QtWidgets import QMessageBox
            QMessageBox.critical(None, "Error", f"No se pudo abrir la herramienta externa:\n{str(e)}")
